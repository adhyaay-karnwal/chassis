...
"""
        ...
        self.assertEqual(
            ...
            verify_supplement_functions(
                ir,
                (
                    "chassis;core.output.diff.hot",
                    "chassis;core.output.diff.inlined",
                ),
                production_module="chassis",
            ),
        )
...
"""
        ...
        with self.assertRaisesRegex(PgsoError, "remained size-shaped"):
            verify_supplement_functions(
                ir,
                ("chassis;core.output.diff.cold",),
                production_module="chassis",
            )

    def test_extracts_only_compatible_functions_in_the_workload_family(self) -> None:
        production = profile_text(
            ("chassis;core.workspace.file_index.scoreAsciiRange", 41, (1, 0)),
            ("chassis;core.workspace.file_index.findWorstSlot", 42, (1,)),
            ("chassis;mem.Allocator.alloc", 43, (1,)),
        )
    ...
    def test_rejects_a_family_without_a_compatible_nonzero_profile(self) -> None:
        production = profile_text(
            ("chassis;core.output.diff.compute", 7, (1,)),
        )
        benchmark = profile_text(
    ...
    def test_normalizes_workload_counts_relative_to_production_cold_cutoff(self) -> None:
        production = profile_text(
            ("chassis;core.output.diff.compute", 7, (100, 50, 25, 25)),
        )
        benchmark = profile_text(
            (
    ...
    def test_rejects_malformed_counter_count(self) -> None:
        production = profile_text(("chassis;core.output.diff.compute", 7, (1,)))
        benchmark = profile_text(
    ...
    def test_maps_the_complete_production_summary_to_a_benchmark_safely(self) -> None:
        production = profile_text(
            ("chassis;core.output.diff.same", 7, (100, 50)),
            ("chassis;core.output.diff.changed", 8, (25,)),
            ("chassis;core.sessions.unused", 9, (10,)),
        )
        benchmark = profile_text(
