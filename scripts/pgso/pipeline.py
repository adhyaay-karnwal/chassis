...
CHASSIS_MACHINE_OUTLINER_FLAGS = (
...
ARTIFACT_LAYOUTS = {
    "chassis": (None, "chassis", "chassis.bc"),
...
def profile_use_argv(
    ...
    flags = USE_FLAGS if paths.selector == "chassis" else BENCHMARK_USE_FLAGS
    return (
        str(toolchain.opt),
        *flags,
...
def candidate_object_argv(
    ...
    outliner_flags = CHASSIS_MACHINE_OUTLINER_FLAGS if paths.selector == "chassis" else ()
    return (
        str(toolchain.llc),
        "-filetype=obj",
        "-O=2",
