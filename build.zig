...
pub fn build(b: *std.Build) void {
    ...
    const run_step = b.step("run", "Run chassis");
    ...
    run_exe_tests.setEnvironmentVariable(
        "CHASSIS_TEST_PRODUCT_EXE",
        b.getInstallPath(.bin, "chassis"),
    );
...
fn addWasmArtifact(
    ...
    const description = switch (surface) {
        .core => "Build the headless chassis WebAssembly artifact",
        .term => "Build the terminal chassis WebAssembly artifact",
        .none => unreachable,
    };
...
fn addNapiArtifact(
    ...
    const install = b.addInstallArtifact(lib, .{ .dest_sub_path = "libchassis.node" });
    const step = b.step("libchassis-napi", "Build the libchassis Node-API core addon");
