#!/usr/bin/env bash
#
# Startup latency benchmarks for chassis.
#
...
CHASSIS_BIN="${REPO_ROOT}/zig-out/bin/chassis"
RESULTS_DIR="${REPO_ROOT}/benchmarks/results"
SESSION_FIXTURE_ROOT="${TMPDIR:-/tmp}/chassis-session-list-benchmark-$$"
...
if [ "$SKIP_BUILD" = false ]; then
  echo "Building chassis (ReleaseSafe)..."
  (cd "$REPO_ROOT" && zig build -Doptimize=ReleaseSafe)
fi

if [ ! -x "$CHASSIS_BIN" ]; then
  echo "error: chassis binary not found at $CHASSIS_BIN"
  exit 1
fi
...
mkdir -p "$GENERAL_FIXTURE_HOME/.chassis"
chmod 700 "$GENERAL_FIXTURE_HOME/.chassis"
printf '%s\n' \
  ...
  > "$GENERAL_FIXTURE_HOME/.chassis/settings.json"
chmod 600 "$GENERAL_FIXTURE_HOME/.chassis/settings.json"
...
echo "=== chassis startup benchmarks ==="
echo "binary: $CHASSIS_BIN"
echo "runs:   $RUNS (warmup: $WARMUP)"
echo ""

# Baseline: process launch floor on this host. This is reported for context;
# the budget checker still enforces each chassis command's raw wall-clock mean.
...
# Benchmark 0: chassis startup (CLI dispatch, no TTY needed)
echo "--- chassis (startup) ---"
HOME="$GENERAL_FIXTURE_HOME" CHASSIS_BENCH=1 hyperfine \
  "${SHELL_OPTS[@]}" \
  --runs "$RUNS" \
  --warmup "$WARMUP" \
  --export-json "${RESULTS_DIR}/startup.json" \
  --command-name "chassis (startup)" \
  "$CHASSIS_BIN"

echo ""

# Benchmark 1: chassis help (minimal startup path)
echo "--- chassis help ---"
HOME="$GENERAL_FIXTURE_HOME" hyperfine \
  "${SHELL_OPTS[@]}" \
  --runs "$RUNS" \
  ...
  --command-name "chassis help" \
  "$CHASSIS_BIN help"

echo ""

# Benchmark 2: chassis status --json (config load + JSON serialize)
echo "--- chassis status --json ---"
HOME="$GENERAL_FIXTURE_HOME" hyperfine \
  "${SHELL_OPTS[@]}" \
  --runs "$RUNS" \
  --warmup "$WARMUP" \
  --export-json "${RESULTS_DIR}/status.json" \
  --command-name "chassis status --json" \
  "$CHASSIS_BIN status --json"

echo ""

# Benchmark 3: chassis doctor --json (system checks)
echo "--- chassis doctor --json ---"
HOME="$GENERAL_FIXTURE_HOME" hyperfine \
  "${SHELL_OPTS[@]}" \
  --runs "$RUNS" \
  --warmup "$WARMUP" \
  --export-json "${RESULTS_DIR}/doctor.json" \
  --command-name "chassis doctor --json" \
  "$CHASSIS_BIN doctor --json"

echo ""

# Benchmark 4: chassis sessions --json (file I/O path)
echo "--- chassis sessions --json ---"
HOME="$SESSION_FIXTURE_HOME" hyperfine \
  "${SHELL_OPTS[@]}" \
  --runs "$RUNS" \
  --warmup "$WARMUP" \
  --export-json "${RESULTS_DIR}/sessions.json" \
  --command-name "chassis sessions --json" \
  "$CHASSIS_BIN sessions --json"
