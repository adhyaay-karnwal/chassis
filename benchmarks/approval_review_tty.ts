...
const DIFF_LINES = (() => {
  const value = Number(process.env.CHASSIS_APPROVAL_PROFILE_DIFF_LINES ?? "50000");
  if (!Number.isSafeInteger(value) || value <= 0) {
    throw new Error(`invalid CHASSIS_APPROVAL_PROFILE_DIFF_LINES: ${JSON.stringify(value)}`);
  }
  return value;
})();
...
async function waitForPaneDeath(session: TmuxSession, timeoutMs: number): Promise<number | null> {
  ...
  throw new Error("timed out waiting for chassis process exit");
}

const binary = realpathSync(requiredEnv("CHASSIS_APPROVAL_PROFILE_BIN"));
const outRoot = requiredEnv("CHASSIS_APPROVAL_PROFILE_OUT");
const cycles = positiveInteger(process.env.CHASSIS_APPROVAL_PROFILE_CYCLES, DEFAULT_CYCLES);
...
try {
  session = await TmuxSession.create({
    ...
    env: {
      ...
      CHASSIS_SYNC_UPDATES: "1",
      CHASSIS_TRACE_LOG: tracePath,
      CHASSIS_TRACE_SCOPES: "permission,input,frame_render,terminal_diff",
      NO_COLOR: "1",
    },
    stderrPath,
    width: 120,
  ...
  assert(readFileSync(stderrPath, "utf8") === "", "chassis wrote to stderr");
  ...
  assert(exitStatus === 0, `chassis exit status was ${exitStatus}`);
  passed = true;
