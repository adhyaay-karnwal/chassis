...
function createIsolatedRoot(domains = ["example.com"]) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "chassis-web-fetch-live-")));
...
describe.skipIf(process.env.CHASSIS_WEB_FETCH_LIVE !== "1")("live web_fetch public URL", () => {
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const sessionDir = join(root.home, ".chassis", "sessions", json.session_id);
