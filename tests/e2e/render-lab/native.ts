...
export const NATIVE_GATE_ENV = "CHASSIS_RENDER_LAB_NATIVE";
export const NATIVE_COMMAND_K_ENV = "CHASSIS_RENDER_LAB_NATIVE_COMMAND_K";
export const NATIVE_CLIPBOARD_ENV = "CHASSIS_RENDER_LAB_NATIVE_ALLOW_CLIPBOARD";
...
const NATIVE_SCENARIOS: NativeScenario[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      "Requires the additional CHASSIS_RENDER_LAB_NATIVE_COMMAND_K=1 gate because it clears scrollback.",
    ],
  },
  {
...
async function launchFx(
  ...
  await controller.sendText(
    `CHASSIS_RECORD=${shQuote(context.manifest.tapePath)} CHASSIS_RECORD_INPUT=1 ${shQuote(CHASSIS_BIN)}`,
  );
  await capture(context, controller, `${label}-chassis-launch-requested`);
  await waitForText(controller, "Run /help for commands", 30_000);
  await capture(context, controller, `${label}-chassis-prompt-visible`);
}
...
async function quitFx(context: NativeContext, controller: NativeController, label: string): Promise<void> {
  ...
  await capture(context, controller, `${label}-chassis-quit-requested`);
  await waitForText(controller, PROMPT_TEXT, 20_000);
...
function startShellCommand(
  ...
  const env = [
    ...
    `CHASSIS_TRACE_LOG=${shQuote(manifest.traceLogPath)}`,
