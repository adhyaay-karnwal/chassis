...
- Did replaying those bytes through `chassis replay` produce the expected model?
...
CHASSIS_RECORD tape and chassis replay
...
3. Launch the freshly built `zig-out/bin/chassis`.
...
```bash
cd /Users/example/Developer/Fx/chassis-worktree-rendering
zig build
cd tests/e2e
bun run render-lab -- --scenario same-shell-relaunch --runs 1 --out /private/tmp/chassis-render-lab
```

Default test wrapper:

```bash
cd /Users/example/Developer/Fx/chassis-worktree-rendering/tests/e2e
...
```bash
CHASSIS_RENDER_LAB_NATIVE=1 bun run render-lab -- --scenario native-terminal-app-relaunch --runs 1 --out /private/tmp/chassis-native
```

Ghostty and Warp use clipboard-based selected-text capture, so they require an extra explicit gate:

```bash
CHASSIS_RENDER_LAB_NATIVE=1 CHASSIS_RENDER_LAB_NATIVE_ALLOW_CLIPBOARD=1 \
  bun run render-lab -- --scenario native-ghostty-relaunch --runs 1 --out /private/tmp/chassis-native-ghostty
```

The Command-K scenario requires a second gate because it triggers real terminal clear-scrollback behavior:

```bash
CHASSIS_RENDER_LAB_NATIVE=1 CHASSIS_RENDER_LAB_NATIVE_COMMAND_K=1 \
  bun run render-lab -- --scenario native-terminal-app-command-k --runs 1 --out /private/tmp/chassis-native-command-k
```
...
- `replay-summary.json`: structured `chassis replay` output.
...
- Command-K requires System Events and the explicit `CHASSIS_RENDER_LAB_NATIVE_COMMAND_K=1` gate.
...
- Requires `CHASSIS_RENDER_LAB_NATIVE_ALLOW_CLIPBOARD=1`.
...
bun test tui-render-lab.test.ts
bun run render-lab -- --scenario same-shell-relaunch --runs 1 --out /private/tmp/chassis-render-lab
../../zig-out/bin/chassis status --json
git diff --check
```

Use the exact freshly built binary from this checkout. Do not run bare `chassis`.
