...
test("render-lab lists gated Plan B native scenarios", () => {
  ...
  expect(output).toContain("CHASSIS_RENDER_LAB_NATIVE=1");
});

test.skipIf(!existsSync(CHASSIS_BIN))("buffer-system frame benchmark writes timing artifact", () => {
  outDir = mkdtempSync(join(tmpdir(), "chassis-frame-bench-test-"));
...
test.skipIf(!existsSync(CHASSIS_BIN))("buffer-system p95 gate rejects undersampled runs", () => {
  outDir = mkdtempSync(join(tmpdir(), "chassis-frame-bench-samples-test-"));
...
test("native render-lab scenarios require explicit opt-in", () => {
  outDir = mkdtempSync(join(tmpdir(), "chassis-render-lab-gate-test-"));
  const env = { ...process.env };
  delete env.CHASSIS_RENDER_LAB_NATIVE;
  delete env.CHASSIS_RENDER_LAB_NATIVE_COMMAND_K;
  delete env.CHASSIS_RENDER_LAB_NATIVE_ALLOW_CLIPBOARD;
  ...
  expect(result.stderr).toContain("CHASSIS_RENDER_LAB_NATIVE=1");
});

test("render-lab analyzer flags excess gap before transient activity", () => {
  outDir = mkdtempSync(join(tmpdir(), "chassis-render-lab-analyzer-test-"));
...
test("render-lab analyzer recognizes the real chassis prompt glyph", () => {
  outDir = mkdtempSync(join(tmpdir(), "chassis-render-lab-analyzer-test-"));
  const manifest = writeAnalyzerFixture(outDir, [
...
test("render-lab analyzer rejects damaged multiline footer with real hint and cursor", () => {
  ...
  const manifest = writeAnalyzerFixture(outDir, [
    ...
    "  transcript row two",
    "────────────────",
    "run chassis login · opus 4.7 · default",
  ], { row: 3, col: 8 });
...
describe.skipIf(SKIP)("tui: render lab", () => {
  ...
  test(
    ...
    () => {
      outDir = mkdtempSync(join(tmpdir(), "chassis-render-lab-test-"));
      const env = { ...process.env };
      ...
      expect(CHASSIS_BIN).toContain("zig-out/bin/chassis");
      const runDir = output.trim().split(/\s+/).at(-1)!;
...
function runScenarioArtifacts(scenario: string): {
  ...
  outDir = mkdtempSync(join(tmpdir(), "chassis-render-lab-overflow-test-"));
  const env = { ...process.env };
  delete env.AI_GATEWAY_API_KEY;
  delete env.VERCEL_OIDC_TOKEN;
  const output = execFileSync(
