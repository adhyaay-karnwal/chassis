...
const KEEP_LARGE_SKILL_ARTIFACTS =
  process.env.CHASSIS_TUI_RESIZE_KEEP_ARTIFACTS === "1";
...
async function createResizeSession(
  ...
  let home = env.HOME;
  if (!home) {
    const root = mkdtempSync(join(tmpdir(), "chassis-resize-home-"));
    tempDirs.push(root);
    home = join(root, "home");
    env.HOME = home;
  }
...
function createStderrPath(label: string): string {
  const dir = mkdtempSync(join(tmpdir(), `chassis-resize-${label}-`));
  tempDirs.push(dir);
...
async function launchRecordedSurfaceSession(
  ...
  const root = realpathSync(
    mkdtempSync(join(tmpdir(), `chassis-footer-surface-${label}-`)),
  );
  ...
  const active = await createResizeSession({
    ...
    env: {
      ...
      CHASSIS_RECORD: join(root, "session.fxtape"),
...
test("selected slash row ignores the welcome header help hint", () => {
  const header = `${SELECTED_COMPLETION_SGR}chassis\x1b[0m\x1b[38;5;245m v0.3.27 · Run /help for commands`;
...
async function waitForLiveScrollbackText(
  ...
  while (Date.now() < deadline) {
    ...
    if (status.dead) {
      throw new Error(
        `chassis exited with status ${status.status} while waiting for ${JSON.stringify(needle)}.\nScrollback:\n${last}`,
      );
    }
...
function expectSkillsMenuGrid(
  ...
  expect(text).toContain("chassis · Global");
...
function matchingTestSessions(): string[] {
  const prefix = `chassis-test-${process.pid}-`;
  try {
    return execFileSync(
      "tmux",
...
function createSkillFixture(
  ...
  const root = realpathSync(
    mkdtempSync(join(tmpdir(), `chassis-resize-${rootLabel}-${attempt}-`)),
  );
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const skillsRoot = join(home, ".chassis", "skills");
  ...
  const fixtures = Array.from({ length: count }, (_, index) => {
    ...
    const marker = `chassis-gauntlet-${attemptName}-${suffix}`;
...
function createFileApprovalRoot() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "chassis-resize-file-approval-")));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(join(home, ".chassis"), { recursive: true });
  mkdirSync(workspace, { recursive: true });
...
describe.skipIf(SKIP)("tui: resize", () => {
  test(
    ...
    async () => {
      ...
      const headerRow = grid.findIndex((line) => line.includes("chassis v"));
  ...
  test(
    ...
    async () => {
      session = await createResizeSession({ cmd: "sleep 120" });
      const dir = mkdtempSync(join(tmpdir(), "chassis-tmux-gate-failure-"));
      tempDirs.push(dir);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-long-transcript-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          CHASSIS_AUTO_UPGRADE: "0",
          CHASSIS_TRACE_LOG: tracePath,
          CHASSIS_TRACE_SCOPES: "frame_schedule,frame_diff,frame_commit,scroll,resize",
          NO_COLOR: "1",
        },
        width: 120,
        height: 34,
      ...
      expect(scrollback.match(/chassis v\d+\.\d+\.\d+\b/g)).toHaveLength(1);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-live-command-scrollback-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), "chassis-assistant-retention-")));
      if (!KEEP_LARGE_SKILL_ARTIFACTS) tempDirs.push(root);
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      ...
      mkdirSync(join(home, ".chassis"), { recursive: true });
      mkdirSync(workspace);
      writeFileSync(join(home, ".chassis", "settings.json"), JSON.stringify({ collapse_tool_calls: true }));
      ...
      session = await createResizeSession({
        cmd: CHASSIS_BIN, cwd: workspace, width: 168, height: 75,
        ...
        env: {
          ...
          VERCEL_OIDC_TOKEN: undefined, CHASSIS_DISABLE_KEYCHAIN: "1",
          CHASSIS_E2E_DISABLE_DOTENV: "1", CHASSIS_SOUND: "0", CHASSIS_AUTO_UPGRADE: "0",
          CHASSIS_SKIP_ONBOARDING: "1", CHASSIS_MODEL: FAKE_GATEWAY_MODEL,
          CHASSIS_GATEWAY_BASE_URL: gateway.baseUrl, CHASSIS_GATEWAY_CHAT_URL: gateway.chatUrl,
          CHASSIS_E2E_GATEWAY_CHAT_URL: gateway.chatUrl,
          CHASSIS_TRACE_SCOPES: "transcript_retention,scroll", CHASSIS_TRACE_LOG: tracePath,
          CHASSIS_RECORD: join(root, "terminal.fxtape"),
        },
      });
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-retention-native-scrollback-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      ...
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-cancel-command-approval-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      const transcriptCopyCounts = (scrollback: string) => ({
        startup: scrollback.match(/chassis v\d+\.\d+\.\d+\b/g)?.length ?? 0,
      ...
      const replay = JSON.parse(
        execFileSync(CHASSIS_BIN, ["replay", tapePath, "--json"], { encoding: "utf8" }),
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-thematic-rule-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-list-paragraph-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          CHASSIS_RECORD: tapePath,
          CHASSIS_TRACE_LOG: tracePath,
          CHASSIS_TRACE_SCOPES: "resize,frame_schedule,scroll",
          NO_COLOR: "1",
        },
        width: 167,
        height: 40,
      ...
      const replay = Bun.spawnSync([CHASSIS_BIN, "replay", tapePath, "--json"], {
        cwd: workspace,
        env: { ...process.env, HOME: home, CHASSIS_SOUND: "0", CHASSIS_AUTO_UPGRADE: "0" },
      });
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-nested-blockquote-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`chassis resize artifacts:\n${roots.join("\n")}`);
        console.log(
  ...
  test(
    ...
    async () => {
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(
          `chassis rapid resize artifacts:\n${results.map(({ root }) => root).join("\n")}`,
        );
      }
    },
    RAPID_SKILL_TIMEOUT,
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-gated-stream-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`chassis gated stream resize artifact:\n${root}`);
      }
    },
    60_000,
  );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-open-picker-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`chassis open picker resize artifact:\n${root}`);
      }
    },
    60_000,
  );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-short-picker-dismissal-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-wide-user-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          CHASSIS_RECORD_INPUT: "1",
          CHASSIS_TRACE_LOG: tracePath,
          CHASSIS_TRACE_SCOPES: "render,resize,transcript,input,worker",
          TMUX: undefined,
        },
        width: 80,
        height: 24,
      ...
      expect(readFileSync(tapePath).byteLength).toBeGreaterThan(0);
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`chassis wide-user resize artifact:\n${root}`);
      }
    },
    TIMEOUT,
  );
  ...
  test(
    ...
    async () => {
      ...
      const first = "CHASSIS_HARD_ROW_ALPHA";
      const second = "CHASSIS_HARD_ROW_BETA";
  ...
  test(
    ...
    async () => {
      ...
      const first = "CHASSIS_SOFT_START";
      const second = "CHASSIS_SOFT_END";
  ...
  test(
    ...
    async () => {
      ...
      const first = "CHASSIS_HARD_4_ROW_ALPHA";
      const second = "CHASSIS_HARD_4_ROW_BETA";
      session = await launchAt(80, 24, { stderrPath });
      await enterHardNewlineInput(session, first, second);
  ...
  test(
    ...
    async () => {
      ...
      const first = "CHASSIS_HARD_9_ROW_ALPHA";
      const second = "CHASSIS_HARD_9_ROW_BETA";
      session = await launchAt(80, 24, { stderrPath });
      await enterHardNewlineInput(session, first, second);
    ...
    async () => {
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          CHASSIS_AUTO_UPGRADE: "0",
          CHASSIS_TRACE_LOG: tracePath,
          CHASSIS_TRACE_SCOPES: "frame_schedule",
          NO_COLOR: "1",
        },
        stderrPath,
        width: 80,
  ...
  test(
    ...
    async () => {
      ...
      expect(restored.match(/chassis v\d+\.\d+\.\d+\b/g)).toHaveLength(1);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "chassis-resize-prepaint-input-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      // Drift the terminal position without updating chassis's shadow grid. Tmux
      ...
      execFileSync(CHASSIS_BIN, ["replay", tapePath, "--frames-dir", replayDir], {
        cwd: workspace,
        stdio: "pipe",
      });
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "chassis-resize-large-paste-"));
      tempDirs.push(dir);
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          CHASSIS_GATEWAY_CHAT_URL: gateway.chatUrl,
          CHASSIS_TRACE_LOG: tracePath,
          CHASSIS_TRACE_SCOPES: "input,worker,resize",
        },
      });
      await session.waitForComposer(10_000);
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "chassis-resize-cpr-paste-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "chassis-resize-cpr-paste-late-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "chassis-resize-probe-timeout-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    "settled resize clears pre-chassis scrollback in both startup modes",
    async () => {
      for (const startupScrollback of [true, false]) {
        const root = mkdtempSync(join(tmpdir(), "chassis-resize-history-reset-"));
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "chassis-theme-reset-replay-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      const stderrPath = join(root, "stderr.log");
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          CHASSIS_RECORD: tapePath,
          CHASSIS_TRACE_LOG: tracePath,
          CHASSIS_TRACE_SCOPES: "theme,frame_schedule,frame_commit,resize",
        },
        stderrPath,
        width: 120,
        height: 40,
