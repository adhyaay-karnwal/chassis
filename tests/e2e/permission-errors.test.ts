...
async function waitForPaneExit(
  ...
  throw new Error(
    `Timed out waiting for chassis ask to exit.\n${await session.captureFullScrollback()}`,
  );
}
...
async function runTtyPromptPermissionsCase(
  ...
  const root = createIsolatedRoot(
    `chassis-${outputMode}-prompt-permissions-${decision}-`,
  );
  ...
  try {
    session = await TmuxSession.create({
      cmd: `${JSON.stringify(CHASSIS_BIN)} ask --${outputMode} --prompt-permissions --no-save "Run the exact ${outputMode} fixture." > ${JSON.stringify(stdoutPath)}`,
      cwd: root.workspace,
      env: permissionEnv(root.home, gateway),
      remainOnExit: true,
    });
    const prompt = await session.waitForText("Approve? [y/N]", TIMEOUT);
    expect(prompt).toContain("chassis wants to run:");
    ...
    if (outputMode === "json") {
      const json = JSON.parse(stdout) as ChassisJson;
...
describe("generic permission typed errors", () => {
  test(
    ...
    async () => {
      const root = createIsolatedRoot("chassis-permission-error-");
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      const root = createIsolatedRoot("chassis-json-auto-prompt-permissions-");
      ...
      try {
        session = await TmuxSession.create({
          cmd: `${JSON.stringify(CHASSIS_BIN)} ask --auto --json --prompt-permissions --no-save "Run the advisory caution fixture." > ${JSON.stringify(stdoutPath)}`,
          cwd: root.workspace,
          env: permissionEnv(root.home, gateway),
          remainOnExit: true,
        });
  ...
  test(
    ...
    async () => {
      ...
      for (const testCase of cases) {
        const root = createIsolatedRoot(
          `chassis-${testCase.mode}-${testCase.optIn ? "opt-in" : "default"}-non-tty-`,
        );
        ...
        try {
          ...
          if (testCase.mode === "json") {
            const json = JSON.parse(result.stdout) as ChassisJson & {
              error: string;
            };
