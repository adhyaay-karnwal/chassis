...
const OAUTH_SERVICE = "CHASSIS_OAUTH_SESSION_V1";
const TEST_ACCOUNT_PREFIX = "chassis-e2e-oauth-";
...
function keychainEnv(home: string, account: string, issuer: string) {
  ...
  return {
    ...
    CHASSIS_E2E_OAUTH_ISSUER_URL: issuer,
    CHASSIS_TRACE_LOG: join(home, "oauth-keychain-trace.log"),
    CHASSIS_TRACE_SCOPES: "auth,keychain",
  };
}
...
keychainTest(
  ...
  async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "chassis-oauth-keychain-readonly-"));
    ...
    const path = join(home, ".chassis", "auth.json");
    const original = readFileSync(path, "utf8");
    chmodSync(path, 0o400);
    try {
      const result = await runFx(["login"], {
        env: { ...keychainEnv(home, account, issuer.issuer), CHASSIS_NO_OPEN_BROWSER: "1", CHASSIS_SOUND: "0" },
        timeoutMs: 3000,
      });
...
keychainTest(
  ...
  async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "chassis-oauth-keychain-migration-"));
    attachSystemKeychain(home);
    const issuer = startOAuthIssuer();
    ...
    try {
      ...
      expect(JSON.parse(first.stdout).auth).toBe("chassis login");
      expect(
        existsSync(join(home, ".chassis", "auth.json")),
      ...
      rmSync(join(home, ".chassis"), { recursive: true, force: true });
      const restarted = await runFx(["status", "--json"], { env, timeoutMs: TIMEOUT });
      expect(restarted.code).toBe(0);
      expect(JSON.parse(restarted.stdout).auth).toBe("chassis login");
...
keychainTest(
  ...
  async () => {
    const account = isolatedAccount();
    const before = productionMetadata();
    const home = mkdtempSync(join(tmpdir(), "chassis-oauth-keychain-failure-"));
    attachSystemKeychain(home);
    const issuer = startOAuthIssuer();
    const cleanup = () => deleteKeychainItem(account);
    activeCleanups.add(cleanup);
