...
test("slash menu selection index ignores the welcome header help hint", () => {
  const header = `${SELECTED_COMPLETION_SGR}chassis\x1b[0m${DIM_SGR} v0.3.27 · Run /help for commands`;
...
describe.skipIf(SKIP)("tui: slash menu", () => {
  ...
  test(
    ...
    async () => {
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("chassis") && !current.includes("Commands"),
        5_000,
      );
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("chassis") && !current.includes("Sessions"),
        5_000,
      );
      ...
      pane = await session.waitForPane(
        (current) =>
          current.includes("resume-helper") &&
          !current.includes("enter use") &&
          !current.includes("chassis needs a model provider"),
        5_000,
      );
  ...
  test(
    ...
    async () => {
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("chassis") && !current.includes("enter open"),
        5_000,
      );
  ...
  test(
    ...
    async () => {
      ...
      expect(tail_history).not.toContain("chassis v0.3.7");
  ...
  test(
    ...
    async () => {
      ...
      await session.sendKeys("C-[");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("chassis") && !current.includes("tab provider"),
        5_000,
      );
