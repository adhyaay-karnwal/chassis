 ...
 * freshly built chassis binary in tmux, arms one observer checkpoint, releases one
...
function createFixtureRoot(autoPermissions = false) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "chassis-ui-observer-fixture-")));
...
function createArtifactDir(keep: boolean): string {
  const configured = process.env.CHASSIS_UI_OBSERVER_ARTIFACT_DIR;
  if (configured) {
    mkdirSync(configured, { recursive: true });
    return realpathSync(configured);
  }
  const root = mkdtempSync(join(tmpdir(), "chassis-ui-observer-"));
...
function gatewayEnv(
  ...
  return {
    ...
    CHASSIS_UI_OBSERVE_DIR: artifactDir,
    CHASSIS_RECORD: join(artifactDir, "session.fxtp"),
    NO_COLOR: "1",
  };
}
...
async function run() {
  ...
  if (!existsSync(CHASSIS_BIN)) {
    throw new Error(`fresh chassis binary is missing: ${CHASSIS_BIN}\nRun: zig build`);
  }
  ...
  const preserveArtifacts = args.keep || process.env.CHASSIS_UI_OBSERVER_ARTIFACT_DIR !== undefined;
  try {
