...
built chassis binary. It is pinned to:
...
`package-lock.json` and installs the pinned official runner. Bun executes chassis's
...
```sh
CHASSIS_SOUND=0 zig build
cd tests/e2e/conformance
npm ci
CHASSIS_SOUND=0 npm test
```
...
starting scenarios when the freshly built `zig-out/bin/chassis` is missing.
...
launches `./zig-out/bin/chassis` without implementing MCP messages or transport
behavior.
