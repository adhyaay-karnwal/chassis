...
pub fn renderTopLevelHelpWithStyle(alloc: Allocator, registry: TopLevelRegistry, columns: usize, version: []const u8, style: HelpStyle) ![]u8 {
    ...
    try writeStyled(&out.writer, style, .brand, "chassis");
    ...
    try writeWrappedStyledLine(&out.writer, "  ", "  ", "chassis [flags]", width, style, .syntax);
    try writeWrappedStyledLine(&out.writer, "  ", "  ", "chassis <command> [...flags] [...args]", width, style, .syntax);
...
pub fn renderTopLevelCommandHelp(alloc: Allocator, registry: TopLevelRegistry, kind: TopLevelKind) ![]u8 {
    ...
    try out.writer.writeAll("chassis ");
...
test "rendered top-level help is a complete CLI navigation page" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, text, "chassis v9.8.7\nFast, native coding agent for the terminal."));
    try std.testing.expect(std.mem.find(u8, text, "chassis starts an interactive session by default.") != null);
    try std.testing.expect(std.mem.find(u8, text, "chassis <command> [...flags] [...args]") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "CHASSIS_EXPERIMENTAL_WORKSPACE_ACCESS=1") == null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "chassis ask \"Explain the changes in this repository\"") != null);
    try std.testing.expect(std.mem.find(u8, text, "chassis session resume last") != null);
    try std.testing.expect(std.mem.find(u8, text, "session resume [last|id]") != null);
    try std.testing.expect(std.mem.find(u8, text, "chassis status --json") != null);
    try std.testing.expect(std.mem.find(u8, text, "Run `chassis <command> --help` for command-specific usage and options.") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "Learn more about chassis:  https://chassis.sh/docs") != null);
    try std.testing.expect(std.mem.find(u8, text, "\nReport a problem:          run `/feedback` inside chassis\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "\n\n\nRun `chassis <command> --help`") == null);
...
test "terminal top-level help adds styling without changing visible content" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, terminal, "\x1b[1mchassis\x1b[0m"));
    ...
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[4mhttps://chassis.sh/docs\x1b[0m") != null);
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[39mrun `/feedback` inside chassis\x1b[0m") != null);
...
test "top-level help renders flags as compact aligned rows" {
    ...
    try std.testing.expect(std.mem.find(u8, wide, "Print the chassis version and exit\n\nExamples:") != null);
...
test "top-level help hides developer recording surfaces" {
    ...
    try std.testing.expect(std.mem.find(u8, replay, "chassis replay") != null);
}
...
test "per-command help renders header usage options and details" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "chassis permissions\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  chassis permissions [--json]") != null);
...
test "per-command help preserves long resume usage without debug recording" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  chassis session resume [last|<id>] | session resume --id <id> | --resume [last|<id>] | resume [last|<id>] | resume --id <id> | --resume-last | --continue | -c | -r | --resume-<id>") != null);
    try std.testing.expect(std.mem.find(u8, text, "Options:") != null);
    try std.testing.expect(std.mem.find(u8, text, "--record") == null);
}
...
test "ACP help documents accepted options" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "chassis acp\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  chassis acp [--model <id>] [--log-file <path>]") != null);
...
test "slash completion descriptions follow completion matches" {
    ...
    try std.testing.expectEqualStrings("open the chassis feedback form", nthSlashCompletionDescription(testSlashRegistry(), "/fee", 0).?);
