...
extern "chassis" fn chassis_config_get(
    id_ptr: [*]const u8,
    id_len: usize,
    out_ptr: [*]u8,
    out_cap: usize,
...
extern "chassis" fn chassis_config_set(
    id_ptr: [*]const u8,
    id_len: usize,
    value_ptr: [*]const u8,
    value_len: usize,
...
pub fn load(alloc: Allocator, id: []const u8) LoadError!?[]u8 {
    ...
    while (capacity <= max_value_bytes) : (capacity *= 2) {
        ...
        const result = chassis_config_get(id.ptr, id.len, value.ptr, value.len);
        if (result >= 0) {
...
/// Persists a value only after chassis has accepted and applied it.
pub fn setAccepted(id: []const u8, value: []const u8) SaveError!void {
    if (chassis_config_set(id.ptr, id.len, value.ptr, value.len) != 0) {
        return error.HostFailure;
    }
}
