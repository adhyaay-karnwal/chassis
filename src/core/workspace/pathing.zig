...
test "external resolver expands home and canonicalizes relative and absolute aliases" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "external");
    try writeTestFile(tmp.dir, "home/chassis-path-fixture.txt", "home");
    try writeTestFile(tmp.dir, "external/chassis-path-fixture.txt", "external");

    const workspace = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "workspace");
    defer alloc.free(workspace);
    const home = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home");
    defer alloc.free(home);
    const home_file = try io_mod.dirRealpathAlloc(arena, tmp.dir, "home/chassis-path-fixture.txt");
    const external_file = try io_mod.dirRealpathAlloc(arena, tmp.dir, "external/chassis-path-fixture.txt");
    ...
    const from_relative = try resolveWorkspaceOrExternalPathWithHome(
        arena,
        workspace,
        "../external/chassis-path-fixture.txt",
        home,
        .existing,
    );
...
test "external resolver follows home symlinks but rejects workspace-relative symlink escapes" {
    ...
    const from_home_link = try resolveWorkspaceOrExternalPathWithHome(
        arena,
        workspace,
        "~/external-link/chassis-path-fixture.txt",
        home,
        .existing,
    );
