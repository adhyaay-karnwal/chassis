...
test "optional typed args return payloads only for matching tags" {
    ...
    const args = try parseToolArgsObject(arena, "{\"name\":\"chassis\",\"enabled\":true,\"count\":3,\"other\":1.25}");

    try std.testing.expectEqualStrings("chassis", optionalStringArg(args, "name").?);
