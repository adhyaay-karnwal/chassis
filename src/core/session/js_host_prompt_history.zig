...
extern "chassis" fn chassis_prompt_history_available() i32;

extern "chassis" fn chassis_prompt_history_load(
...
extern "chassis" fn chassis_prompt_history_append(
    timestamp_ms: i64,
    workspace_ptr: [*]const u8,
    workspace_len: usize,
    text_ptr: [*]const u8,
...
extern "chassis" fn chassis_prompt_history_clear(
    workspace_ptr: [*]const u8,
    workspace_len: usize,
) i32;

pub fn available() bool {
    return chassis_prompt_history_available() == 1;
}
...
fn loadRecent(
    ...
    while (capacity <= max_response_bytes) : (capacity *= 2) {
        ...
        const result = chassis_prompt_history_load(
            workspace_root.ptr,
            workspace_root.len,
            limit,
            buffer.ptr,
...
fn append(
    ...
    return switch (chassis_prompt_history_append(
        timestamp_ms,
        workspace_root.ptr,
        workspace_root.len,
        text.ptr,
...
fn clearWorkspace(
    ...
    if (chassis_prompt_history_clear(workspace_root.ptr, workspace_root.len) != 0) {
        return error.PromptHistoryHostFailure;
    }
}
