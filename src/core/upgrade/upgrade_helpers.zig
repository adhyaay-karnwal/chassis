...
pub const cdn_base = "https://releases.chassis.sh";

pub fn resolveCdnBase() []const u8 {
    if (io_mod.getenv("CHASSIS_E2E_UPGRADE_BASE_URL")) |url| {
...
test "production upgrade base uses the chassis release domain" {
    try std.testing.expectEqualStrings("https://releases.chassis.sh", resolveCdnBase());
}

test "extractChecksumHex parses sha256sum format" {
    const with_filename = "abc123def456  chassis-macos-aarch64.tar.gz\n";
...
test "replaceBinary moves replacement over target path" {
    ...
    try writeTempFile(tmp.dir, "chassis-old", "old");
    try writeTempFile(tmp.dir, "chassis-new", "new");
    const root = try io_mod.dirRealpathAlloc(alloc, tmp.dir, ".");
    defer alloc.free(root);
    const new_path = try std.fs.path.join(alloc, &.{ root, "chassis-new" });
    defer alloc.free(new_path);
    const target_path = try std.fs.path.join(alloc, &.{ root, "chassis-old" });
