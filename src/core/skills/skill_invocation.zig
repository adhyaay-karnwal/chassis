...
test "explicit skill requests report ambiguous names without selecting a source" {
    ...
    const skills = [_]skill_runtime.Skill{
        .{ .name = "review", .description = "workspace", .path = "/workspace/review", .source = .workspace_chassis },
        .{ .name = "review", .description = "global", .path = "/global/review", .source = .global_chassis },
    };
...
test "explicit skills include complete instructions beyond the default chunk" {
    ...
    const skills = [_]skill_runtime.Skill{.{ .name = "workflow", .description = "helper", .path = path, .source = .global_chassis }};
...
test "skill resource defaults preserve whole and legacy reads" {
    ...
    const path = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "workflow");
    defer alloc.free(path);
    const skills = [_]skill_runtime.Skill{.{ .name = "workflow", .description = "", .path = path, .source = .global_chassis }};
...
fn checkPreparedIdentityDiagnostics(alloc: Allocator) !void {
    const skills = [_]skill_runtime.Skill{
        ...
        .{ .name = "workflow", .description = "second", .path = "/skills/second", .source = .global_chassis },
    };
...
fn checkSkillErrorFormattingAllocationFailures(alloc: Allocator) !void {
    ...
    const duplicates = [_]skill_runtime.Skill{
        ...
        .{ .name = "workflow", .description = "B", .path = "/tmp/b/workflow", .source = .global_chassis },
    };
...
test "skill invocation preserves hard allocation failures" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.chassis/skills/workflow");
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.chassis/skills/workflow/SKILL.md", .{});
        defer file.close(io_mod.getIo());
    ...
    const skills_dir = try io_mod.dirRealpathAlloc(alloc, tmp.dir, "home/.chassis/skills");
...
test "skill invocation loads installed skill content" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.chassis/skills/workflow/assets");
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.chassis/skills/workflow/SKILL.md", .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), "---\nname: workflow\ndescription: workflow helper\n---\n\nuse the workflow skill\n");
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.chassis/skills/workflow/assets/data.txt", .{});
        defer file.close(io_mod.getIo());
...
test "stale skill catalogs reject mutated candidates before loading" {
    ...
    inline for (.{ "deleted", "invalid", "renamed" }) |mutation| {
        ...
        if (std.mem.eql(u8, mutation, "deleted")) {
            try tmp.dir.deleteFile(io_mod.getIo(), "home/.chassis/skills/workflow/SKILL.md");
        } else {
            var file = try tmp.dir.createFile(
                io_mod.getIo(),
                "home/.chassis/skills/workflow/SKILL.md",
                .{ .truncate = true },
            );
            defer file.close(io_mod.getIo());
...
const CandidatePathReplacementHook = struct {
    ...
    fn check(raw: *anyopaque) !void {
        ...
        try self.dir.rename(
            "home/.chassis/skills/workflow",
            self.dir.*,
            "home/.chassis/skills/workflow-original",
            io_mod.getIo(),
        );
        try self.dir.rename(
...
test "large skills stay discoverable and explicit overrides can continue past default file bounds" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.chassis/skills/large");
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.chassis/skills/large/SKILL.md", .{});
        defer file.close(io_mod.getIo());
...
test "explicit binding plan preserves supplied order and adds prompt matches once" {
    ...
    const skills = [_]skill_runtime.Skill{
        ...
        .{ .name = "release", .description = "Prepare a release", .path = "/skills/release", .source = .global_chassis },
    };
...
test "explicit invocation preserves configured skill content when discovery is incomplete" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home/workspace");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.chassis/skills/large");
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.chassis/skills/malformed");
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.chassis/skills/malformed/SKILL.md", .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), "---\ndescription: missing name\n---\nMALFORMED BODY");
    }
...
test "skill resources continue on line-safe UTF-8 boundaries and reject unsafe inputs" {
    ...
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.chassis/skills/workflow/assets/binary.bin", .{});
        defer file.close(io_mod.getIo());
    ...
    inline for (.{
        ...
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.chassis/skills/workflow/assets/" ++ fixture[0], .{});
        defer file.close(io_mod.getIo());
        try file.writeStreamingAll(io_mod.getIo(), fixture[1]);
    }
    {
        var file = try tmp.dir.createFile(io_mod.getIo(), "home/.chassis/skills/workflow/assets/cross-chunk.txt", .{});
        defer file.close(io_mod.getIo());
...
test "skill invocation encodes loaded paths and preserves the skill body" {
    ...
    const skill_root = home_root ++ "/.chassis/skills/workflow";
    ...
    defer alloc.free(workspace_root);
    const skills_dir = try io_mod.dirRealpathAlloc(alloc, tmp.dir, home_root ++ "/.chassis/skills");
    defer alloc.free(skills_dir);
    try setTestHome(null);
    defer setTestHome(null) catch {};
...
test "skill invocation preserves strict discovery diagnostics without a profile home" {
    ...
    const workspace_root = try std.fs.path.join(alloc, &.{ canonical_tmp, "chassis-skill-tool-strict-workspace" });
    defer alloc.free(workspace_root);
...
test "ambiguous skill failure uses configured bound and exact omitted count" {
    ...
    const duplicates = [_]skill_runtime.Skill{
        ...
        .{ .name = "workflow", .description = "B", .path = long_location, .source = .global_chassis },
    };
