...
pub const provider_response_format_name = "chassis_vision_evidence";
...
test "Vision result serialization emits the five chassis-owned diagnostics" {
    const Case = struct {
        failure: VisionFailure,
        expected: []const u8,
    };
...
test "Vision provider cannot mint chassis-owned local diagnostic codes" {
    inline for (.{
        "image_unavailable",
        "provider_response_invalid",
        "output_limit_exceeded",
