...
fn loadOptionsFromEnv(alloc: Allocator, workspace_root: []const u8) !Options {
    const trace_log = loadOptionalEnv("CHASSIS_TRACE_LOG");
    const trace_flag = loadOptionalEnv("CHASSIS_TRACE");
    const trace_stderr = loadOptionalEnv("CHASSIS_TRACE_STDERR");
    const trace_scopes = loadOptionalEnv("CHASSIS_TRACE_SCOPES");
...
fn fallbackLogPathForMillis(alloc: Allocator, millis: i64) ![]u8 {
    return std.fmt.allocPrint(alloc, "/tmp/chassis-trace-{d}.log", .{millis});
}
...
test "home default log path uses chassis logs directory" {
    const alloc = std.testing.allocator;
    const path = try defaultLogPathForHome(alloc, "/tmp/fake-home");
    defer alloc.free(path);
    try std.testing.expectEqualStrings("/tmp/fake-home/.chassis/logs/trace.log", path);
}
...
test "fallback default log path uses tmp trace path" {
    ...
    defer alloc.free(path);
    try std.testing.expectEqualStrings("/tmp/chassis-trace-12345.log", path);
}
...
test "keylessJsonPreview reports shape without keys or values" {
    const alloc = std.testing.allocator;
    const preview_text = try keylessJsonPreview(alloc, "{\"CHASSIS_DYNAMIC_PATH\":\"secret.txt\",\"CHASSIS_DYNAMIC_CONTENT\":\"very secret\",\"nested\":{\"CHASSIS_DYNAMIC_TOKEN\":\"abc\"}}");
    defer alloc.free(preview_text);

    try std.testing.expect(std.mem.find(u8, preview_text, "<object_fields=3 values=[") != null);
    try std.testing.expect(std.mem.find(u8, preview_text, "CHASSIS_DYNAMIC_PATH") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "CHASSIS_DYNAMIC_CONTENT") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "CHASSIS_DYNAMIC_TOKEN") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "secret.txt") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "very secret") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "abc") == null);
}
...
test "resolveLogPath resolves absolute and relative paths" {
    const alloc = std.testing.allocator;
    const absolute = try resolveLogPath(alloc, "/tmp/workspace", " \t/tmp/chassis-absolute-trace.log\n");
    defer alloc.free(absolute);
    try std.testing.expectEqualStrings("/tmp/chassis-absolute-trace.log", absolute);
...
test "configureFromEnv leaves tracing disabled without env" {
    resetForTest();
    defer resetForTest();
    try std.testing.expect(io_mod.getenv("CHASSIS_TRACE_LOG") == null);
    try std.testing.expect(io_mod.getenv("CHASSIS_TRACE") == null);
    try std.testing.expect(io_mod.getenv("CHASSIS_TRACE_STDERR") == null);
