...
/// Returns an owned path another process can use to launch chassis. Linux prefers
...
fn testProductExe() ?[]const u8 {
    ...
    const path_z = std.c.getenv("CHASSIS_TEST_PRODUCT_EXE") orelse return null;
...
test "on-disk probe rejects a replaced binary so the peer path falls back" {
    ...
    const victim = try std.fs.path.join(alloc, &.{ dir_path, "chassis-probe" });
    ...
    try std.testing.expect(!onDiskPathIsExecutable("chassis"));
}
