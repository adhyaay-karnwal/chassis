...
pub fn e2eFailIfDurableMutationAttempted() void {
    const enabled = getenv("CHASSIS_E2E_FAIL_ON_DURABLE_MUTATION") orelse return;
...
test "getenv returns null before setEnvironMap" {
    ...
    try std.testing.expect(getenv("CHASSIS_IO_TEST") == null);
}
...
test "getenv returns set value after setEnvironMap" {
    ...
    try environ.put("CHASSIS_IO_TEST", "present");

    setEnvironMap(&environ);
    try std.testing.expectEqualStrings("present", getenv("CHASSIS_IO_TEST").?);
    global_environ = null;
}
...
test "environMap returns borrowed process environment map" {
    ...
    var environ = std.process.Environ.Map.init(std.testing.allocator);
    defer environ.deinit();
    try environ.put("CHASSIS_CORE2_IO_TEST", "present");

    setEnvironMap(&environ);
    const borrowed = environMap().?;
    try std.testing.expectEqualStrings("present", borrowed.get("CHASSIS_CORE2_IO_TEST").?);
    global_environ = null;
}
