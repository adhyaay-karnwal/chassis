...
pub const client_id_env = "CHASSIS_OAUTH_CLIENT_ID";
pub const default_client_id = "cl_zzh5hiOZbwJ9bfqEcYqPIJv3TaPaEYL0";
const e2e_issuer_url_env = "CHASSIS_E2E_OAUTH_ISSUER_URL";
...
const MutationLockProbe = struct {
    ...
    fn tryLock(raw_ctx: ?*anyopaque, file: std.Io.File) anyerror!bool {
        ...
        if (!locked and !self.signaled) {
            signalE2ELockContention(self.chassis_dir);
            self.signaled = true;
        }
        return locked;
    }
...
fn signalE2ELockContention(chassis_dir: std.Io.Dir) void {
    const enabled = io_mod.getenv("CHASSIS_E2E_AUTH_LOCK_CONTENTION") orelse return;
    if (!std.mem.eql(u8, enabled, "1")) return;
    var file = chassis_dir.createFile(io_mod.getIo(), e2e_lock_contention_file_name, .{
        .truncate = true,
...
fn observeAuthFile(alloc: Allocator, chassis_dir: *std.Io.Dir) !FileObservation {
    var file = chassis_dir.openFile(io_mod.getIo(), auth_file_name, .{
        .mode = .read_only,
...
fn authFileExists(chassis_dir: *std.Io.Dir) !bool {
    _ = chassis_dir.statFile(io_mod.getIo(), auth_file_name, .{}) catch |err| switch (err) {
...
const NativeMutation = struct {
    ...
    pub fn requireWritable(self: *Mutation) error{CredentialStorageUnavailable}!void {
        if (self.backend == .profile_file or self.authority == .profile_file) {
            _ = try session_presence.requireWritableInDir(self.chassis_dir.dir, auth_file_name);
        }
    }
    ...
    pub fn load(self: *Mutation, alloc: Allocator) !?Session {
        if (self.backend == .profile_file) {
            ...
            return loadFromDir(alloc, &self.chassis_dir.dir);
        }
    ...
    fn saveFile(self: *Mutation, alloc: Allocator, session: Session) !void {
        ...
        try io_mod.durableReplaceVerified(alloc, &self.chassis_dir, auth_file_name, text);
    }
    ...
    pub fn delete(self: *Mutation, alloc: Allocator) !DeleteResult {
        ...
        const file_outcome = deleteAuthFile(&self.chassis_dir.dir, .{}) catch {
            result.local_cleanup_failed = true;
            return result;
        };
        switch (file_outcome) {
    ...
    fn loadKeychainResolved(self: *Mutation, alloc: Allocator) !?Session {
        var file = try observeAuthFile(alloc, &self.chassis_dir.dir);
    ...
    fn saveKeychainResolved(self: *Mutation, alloc: Allocator, session: Session) !void {
        if (self.authority == .unresolved) {
            self.authority = if (try authFileExists(&self.chassis_dir.dir)) .profile_file else .keychain;
        }
    ...
    fn publishAndCleanup(self: *Mutation, alloc: Allocator, session: Session) !bool {
        ...
        const outcome = deleteAuthFile(&self.chassis_dir.dir, .{}) catch |err| {
            const stat = self.chassis_dir.dir.statFile(io_mod.getIo(), auth_file_name, .{}) catch |stat_err| switch (stat_err) {
        ...
        return switch (outcome) {
            .deleted => true,
            .missing => blk: {
                try io_mod.syncVerifiedDir(self.chassis_dir.dir);
                break :blk true;
            },
...
fn loadFromDir(alloc: Allocator, chassis_dir: *std.Io.Dir) !?Session {
    var file = chassis_dir.openFile(io_mod.getIo(), auth_file_name, .{
        .mode = .read_only,
        .allow_directory = false,
        .follow_symlinks = false,
...
fn beginExistingNativeMutation() !?Mutation {
    ...
    const chassis_dir = openExistingPrivateChassisDir(&home_dir) catch |err| switch (err) {
        error.FileNotFound => return null,
        else => return session_presence.storageError(auth_file_name, err),
    };
    return @as(?Mutation, try lockMutation(chassis_dir));
}
...
fn beginMutation() !Mutation {
    ...
    const chassis_dir = try io_mod.openOrCreateVerifiedPrivateDir(&home_dir, profile_paths.root_dir_name);
    return lockMutation(chassis_dir);
}

fn lockMutation(open_fx_dir: io_mod.VerifiedDir) !Mutation {
    var probe = MutationLockProbe{ .chassis_dir = open_fx_dir.dir };
...
fn openExistingPrivateChassisDir(home_dir: *io_mod.VerifiedDir) !io_mod.VerifiedDir {
...
fn deleteAuthFile(chassis_dir: *std.Io.Dir, ops: io_mod.DurableOps) !DeleteOutcome {
    chassis_dir.deleteFile(io_mod.getIo(), auth_file_name) catch |err| switch (err) {
        error.FileNotFound => return .missing,
        else => return err,
    };
    ops.sync_dir(ops.ctx, chassis_dir.*) catch return .deleted_not_durable;
    return .deleted;
}
