...
pub const Mutation = struct {
    ...
    pub fn delete(self: *Mutation) !DeleteOutcome {
        self.chassis_dir.dir.deleteFile(io_mod.getIo(), auth_file_name) catch |err| switch (err) {
            error.FileNotFound => return .missing,
            else => return err,
        };
        const durable: io_mod.DurableOps = .{};
        durable.sync_dir(durable.ctx, self.chassis_dir.dir) catch return .deleted_not_durable;
        return .deleted;
    }
};
...
pub fn beginExistingMutation() !?Mutation {
    ...
    return try lockMutation(chassis_dir);
}
...
fn lockMutation(open_fx_dir: io_mod.VerifiedDir) !Mutation {
    ...
    return .{ .chassis_dir = chassis_dir, .lock = lock };
}
