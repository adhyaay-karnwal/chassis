...
test "document append preparation preserves presentation boundaries" {
    const open = "\x1b]8;id=chassis-42;https://example.com\x1b\\";
    const raw = "old\nnew";
...
test "transcript surface painter renders welcome plus user rows" {
    const alloc = std.testing.allocator;
    var batch = try transcriptTestBatch(alloc, "chassis welcome\n\n╭ user prompt\n╰ done", 24);
    defer batch.deinit(alloc);
    ...
    defer target.deinit();
    try expectGridContains(&target, alloc, "chassis welcome");
