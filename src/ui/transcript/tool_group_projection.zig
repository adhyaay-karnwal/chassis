...
const cancellation_follow_up = " · What can chassis do differently?";
...
test "minimal tool group keeps cancellation in the header and child row" {
    const alloc = std.testing.allocator;
    const entries = [_]TranscriptEntry{
        .{ .raw_bytes = .{ .id = 1, .bytes = "■ Cancelled sleep 30 · What can chassis do differently?", .class = .tool_status } },
    };
    const details = [_]ToolDetailRecord{
    ...
    try std.testing.expectEqualStrings(
        "● 1 tool call · 1 command · 1 cancelled\n" ++
            "└ Cancelled sleep 30\n\n" ++
            "■ Cancelled sleep 30 · What can chassis do differently?",
        projection.entry_actions.items[0].override.bytes,
    );
}
