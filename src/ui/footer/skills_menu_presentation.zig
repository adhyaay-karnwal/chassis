...
fn skillSourceScopeLabel(source: skill_runtime.SkillSource) []const u8 {
    return switch (source) {
        .global_chassis => "chassis · Global",
        .workspace_chassis => "chassis · Workspace",
        .workspace_shared => "chassis · Workspace",
...
test "skills menu labels native workspace skills with lowercase product name" {
    try std.testing.expectEqualStrings("chassis · Workspace", skillSourceScopeLabel(.workspace_chassis));
}

test "skills menu renders source tabs and single-line results" {
    const alloc = std.testing.allocator;
    const skills = [_]skill_runtime.Skill{
        .{ .name = "managed", .description = "Managed skill description.", .path = "/tmp/managed/SKILL.md", .source = .global_chassis },
...
test "skills menu narrow header always shows the active source" {
    ...
    inline for (.{ @as(u16, 20), @as(u16, 21) }) |width| {
        ...
        var chassis_prepared = try prepareSkillsMenu(alloc, .{
            .active = true,
            .items = &skills,
            .source_filter = .chassis,
        }, 3);
        defer chassis_prepared.deinit(alloc);
        var chassis_header = try composeSkillsMenuRow(alloc, chassis_prepared, 0, width);
        defer chassis_header.deinit(alloc);
        try std.testing.expect(std.mem.find(u8, chassis_header.items, "[chassis]") != null);
        try std.testing.expect(std.mem.find(u8, chassis_header.items, "[Fx]") == null);
        try std.testing.expect(
            display_width.visibleWidthIgnoringAnsi(chassis_header.items) <= width,
        );
    }
}
...
test "skills menu navigation budget counts whole items" {
    const skills = [_]skill_runtime.Skill{
        .{ .name = "one", .description = "one", .path = "/tmp/one", .source = .global_chassis },
        .{ .name = "two", .description = "two", .path = "/tmp/two", .source = .global_chassis },
        .{ .name = "three", .description = "three", .path = "/tmp/three", .source = .global_chassis },
        .{ .name = "four", .description = "four", .path = "/tmp/four", .source = .global_chassis },
    };
...
test "prepared skills menu borrows one indexed query without allocating" {
    const skills = [_]skill_runtime.Skill{
        .{ .name = "ignored", .description = "", .path = "/skills/ignored", .source = .global_chassis },
...
test "skills menu keeps the selected whole item in its window" {
    const alloc = std.testing.allocator;
    const skills = [_]skill_runtime.Skill{
        .{ .name = "one", .description = "one description", .path = "/tmp/one", .source = .global_chassis },
        .{ .name = "two", .description = "two description", .path = "/tmp/two", .source = .global_chassis },
        .{ .name = "three", .description = "three description", .path = "/tmp/three", .source = .global_chassis },
    };
    const projection: SkillsMenuProjection = .{
        .active = true,
        .items = &skills,
...
test "prepared skills menu aligns sources beside the widest visible name" {
    ...
    const skills = [_]skill_runtime.Skill{
        ...
        .{ .name = "managed", .description = "managed", .path = "/tmp/managed", .source = .global_chassis },
