...
extern "chassis" fn chassis_http_stream_open(
    method_ptr: [*]const u8,
    method_len: usize,
    url_ptr: [*]const u8,
    url_len: usize,
...
extern "chassis" fn chassis_http_stream_status(handle: i32, status_out: *u16) i32;
extern "chassis" fn chassis_http_stream_next(handle: i32, out_ptr: [*]u8, out_cap: usize) i32;
extern "chassis" fn chassis_http_stream_close(handle: i32) void;
...
fn open(_: ?*anyopaque, method: []const u8, url: []const u8, headers: []const u8, body: []const u8) !i32 {
    const handle = chassis_http_stream_open(method.ptr, method.len, url.ptr, url.len, headers.ptr, headers.len, body.ptr, body.len);
    if (handle < 0) return error.HostStreamFailed;
    return handle;
}

fn status(_: ?*anyopaque, handle: i32, status_out: *u16) i32 {
    return chassis_http_stream_status(handle, status_out);
}

fn next(_: ?*anyopaque, handle: i32, out: []u8) i32 {
    return chassis_http_stream_next(handle, out.ptr, out.len);
}

fn close(_: ?*anyopaque, handle: i32) void {
    chassis_http_stream_close(handle);
}
