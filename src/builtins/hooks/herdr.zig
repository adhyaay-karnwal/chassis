...
//! ignored so the integration cannot block or terminate an chassis session.
...
const source = "custom:chassis";
const agent_name = "chassis";
...
pub const Client = struct {
    ...
    pub fn shouldEnable(
        chassis_herdr: ?[]const u8,
        socket_path: ?[]const u8,
        pane_id: ?[]const u8,
    ) bool {
        if (chassis_herdr) |val| {
    ...
    pub fn initFromEnv(self: *Client, alloc: std.mem.Allocator) void {
        ...
        if (!shouldEnable(io_mod.getenv("CHASSIS_HERDR"), socket_path, pane_id)) {
            debug_trace.logf("herdr", "disabled socket={s} pane={s} chassis_herdr={s}", .{
                socket_path orelse "(unset)",
                pane_id orelse "(unset)",
                io_mod.getenv("CHASSIS_HERDR") orelse "(unset)",
            });
            return;
        }
    ...
    pub fn deinit(self: *Client) void {
        ...
        // stale idle "chassis" label behind.
        self.release();
    ...
    /// Report chassis's session id so herdr can associate the pane with a resumable
    ...
    /// Name the pane/agent so herdr lists chassis even while idle. Best-effort;
    ...
    /// Remove chassis from the pane on exit: drop the reported agent and clear the
    /// label/name. Without this the pane keeps showing "chassis" after chassis is gone.
...
test "shouldEnable honors CHASSIS_HERDR opt-out" {
...
test "report_agent serializes a single newline-delimited json line" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"7\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:chassis\",\"agent\":\"chassis\",\"state\":\"working\"," ++
...
test "report_agent omits custom_status when null" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"1\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:chassis\",\"agent\":\"chassis\",\"state\":\"idle\"}}\n",
        out.written(),
    );
}
...
test "report_agent escapes pane id" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"2\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"pane\\\"x\"," ++
            "\"source\":\"custom:chassis\",\"agent\":\"chassis\",\"state\":\"blocked\"}}\n",
        out.written(),
    );
}
...
test "pane.rename labels the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeRename(&out.writer, 4, "pane.rename", "pane_id", "w1:p1", "label", "chassis");
    try std.testing.expectEqualStrings(
        "{\"id\":\"4\",\"method\":\"pane.rename\",\"params\":{\"pane_id\":\"w1:p1\",\"label\":\"chassis\"}}\n",
        out.written(),
    );
}
...
test "agent.rename names the agent" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeRename(&out.writer, 5, "agent.rename", "target", "w1:p1", "name", "chassis");
    try std.testing.expectEqualStrings(
        "{\"id\":\"5\",\"method\":\"agent.rename\",\"params\":{\"target\":\"w1:p1\",\"name\":\"chassis\"}}\n",
        out.written(),
    );
}
...
test "clear_agent_authority removes chassis from the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeClearAuthority(&out.writer, 7, "w1:p1");
    try std.testing.expectEqualStrings(
        "{\"id\":\"7\",\"method\":\"pane.clear_agent_authority\",\"params\":{\"pane_id\":\"w1:p1\",\"source\":\"custom:chassis\"}}\n",
        out.written(),
    );
}
...
test "report_agent_session serializes session identity" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"3\",\"method\":\"pane.report_agent_session\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:chassis\",\"agent\":\"chassis\",\"agent_session_id\":\"session-42\"}}\n",
        out.written(),
    );
}
