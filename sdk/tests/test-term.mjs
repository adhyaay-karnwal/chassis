...
const runtime = await createChassisTerminal({
  ...
  traceWasi: process.env.CHASSIS_WASI_TRACE === "1",
  stderr(bytes) { process.stderr.write(bytes); },
});
await Promise.race([
  runtime.interactive,
  new Promise((_, reject) => setTimeout(() => reject(new Error("timed out waiting for chassis-term to become interactive")), 5000)),
]);
...
while (streamStartedAt === undefined) {
  if (performance.now() >= deadline) throw new Error("timed out waiting for continuous chassis-term response");
  await new Promise((resolve) => setTimeout(resolve, 10));
}
...
while (
  ...
  if (performance.now() >= steeringDeadline) throw new Error("timed out waiting for steered chassis-term response");
  await new Promise((resolve) => setTimeout(resolve, 10));
}
releaseFirstStream();
while (streamFinishedAt === undefined) {
  if (performance.now() >= steeringDeadline) throw new Error("timed out releasing held chassis-term response");
  await new Promise((resolve) => setTimeout(resolve, 10));
}
